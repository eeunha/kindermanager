...
function onSuccess(position) {
	...
	insertLocation(latit, longi);

	//
	// $('#box1').html("위도 : " + position.coords.latitude + "<br/>" + "경도 : " + position.coords.longitude);
	// loadGoogleMap(position.coords.latitude, position.coords.longitude);

}

function onSuccess_p(lat, lng) {
	$('#box1').html("위도 : " + lat + "<br/>" + "경도 : " + lng);
...
function loadGoogleMap(lat, lng) {
	...
	$('#mapArea1').gmap('addMarker', {
		'position' : latlng
	}).click(function() {
		$('#mapArea1').gmap('openInfoWindow', {
			'content' : '현재 위치'
		}, this);
	});
}
